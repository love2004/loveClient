...
import me.love2004.porn.modules.client.Managers;
import me.love2004.porn.modules.client.ServerModule;
import me.love2004.porn.modules.combat.AutoCrystal;
import me.love2004.porn.util.GLUProjection;
...
public class EventManager
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!EventManager.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals(EventManager.mc.player)) {
            loveClient.potionManager.update();
            loveClient.totemPopManager.onUpdate();
            loveClient.inventoryManager.update();
            loveClient.holeManager.update();
            loveClient.safetyManager.onUpdate();
            loveClient.moduleManager.onUpdate();
            loveClient.moduleManager.FastonUpdate();
            loveClient.timerManager.update();
            if (this.timer.passedMs(Managers.getInstance().moduleListUpdates.getValue().intValue())) {
                loveClient.moduleManager.sortModules(true);
                loveClient.moduleManager.alphabeticallySortModules();
                this.timer.reset();
            }
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        loveClient.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        loveClient.moduleManager.onLogout();
        loveClient.totemPopManager.onLogout();
        loveClient.potionManager.onLogout();
    }
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        loveClient.moduleManager.onTick();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            loveClient.baritoneManager.onUpdateWalkingPlayer();
            loveClient.speedManager.updateValues();
            loveClient.rotationManager.updateRotations();
            loveClient.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            loveClient.rotationManager.restoreRotations();
            loveClient.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        loveClient.serverManager.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(EventManager.mc.world) instanceof EntityPlayer) {
                ...
                loveClient.totemPopManager.onTotemPop(player);
                loveClient.potionManager.onTotemPop(player);
            }
            ...
            loveClient.serverManager.update();
        }
    }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        EventManager.mc.profiler.startSection("loveClient");
        ...
        loveClient.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            loveClient.textManager.updateResolution();
        }
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            loveClient.moduleManager.onRender2D(render2DEvent);
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    loveClient.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
