...
import me.love2004.porn.modules.misc.ChatModifier;
...
extends Gui {
    ...
    private void drawRectHook(int left, int top, int right, int bottom, int color) {
        Gui.drawRect((int)left, (int)top, (int)right, (int)bottom, (int)(ChatModifier.getInstance().isEnable() && ChatModifier.getInstance().clean.getValue() != false ? 0 : color));
    }
    ...
    private int drawStringWithShadow(FontRenderer fontRenderer, String text, float x, float y, int color) {
        if (text.contains("\u00a7+")) {
            ...
            loveClient.textManager.drawRainbowString(text, x, y, Color.HSBtoRGB(Colors.INSTANCE.hue, 1.0f, 1.0f), 100.0f, true);
        } else {
    ...
    public int drawnChatLinesSize(List<ChatLine> list) {
        return ChatModifier.getInstance().isEnable() && ChatModifier.getInstance().infinite.getValue() != false ? -2147483647 : list.size();
    }
