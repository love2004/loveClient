...
import me.love2004.porn.gui.components.items.buttons.ModuleButton;
...
public class PhobosGui
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : loveClient.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    loveClient.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
