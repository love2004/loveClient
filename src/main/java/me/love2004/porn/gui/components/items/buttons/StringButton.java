...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (ClickGui.getInstance().rainbow.getValue().booleanValue()) {
            ...
            RenderUtil.drawRect(this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? loveClient.colorManager.getColorWithAlpha(loveClient.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : loveClient.colorManager.getColorWithAlpha(loveClient.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())) : (!this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515));
        }
        if (this.isListening) {
            loveClient.textManager.drawStringWithShadow(this.currentString.getString() + loveClient.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            loveClient.textManager.drawStringWithShadow((this.setting.shouldRenderName() ? this.setting.getName() + " " + "\u00a77" : "") + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
