package me.love2004.porn.gui.font;
...
public class CustomFont
    ...
    public float drawString(String textIn, double xI, double yI, int color, boolean shadow) {
        String text = Media.getInstance().isEnable() && Media.getInstance().changeOwn.getValue() != false ? textIn.replace(Media.getPlayerName(), Media.getInstance().ownName.getValue()) : textIn;
        double x = xI;
        double y = yI;
        if (FontMod.getInstance().isEnable() && !FontMod.getInstance().shadow.getValue().booleanValue() && shadow) {
            x -= 0.5;
            y -= 0.5;
        }
        x -= 1.0;
