...
import me.love2004.porn.manager.FileManager;
...
public class AutoGG
        extends Module {
    private static final String path = "loveClient/autogg.txt";
    ...
    public void onAttackEntity(AttackEntityEvent event) {
        if (event.getTarget() instanceof EntityPlayer && !loveClient.friendManager.isFriend(event.getEntityPlayer())) {
    ...
    public void onSendAttackPacket(PacketEvent.Send event) {
        ...
        if (event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(AutoGG.mc.world) instanceof EntityPlayer && !loveClient.friendManager.isFriend((EntityPlayer) packet.getEntityFromWorld(AutoGG.mc.world))) {
