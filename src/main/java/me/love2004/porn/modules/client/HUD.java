...
import me.love2004.porn.modules.misc.ToolTips;
import me.love2004.porn.setting.Setting;
import me.love2004.porn.manager.TextManager;
...
public class HUD
    ...
    private static final ResourceLocation codHitmarker = new ResourceLocation("loveClient", "cod_hitmarker");
    public static final SoundEvent COD_EVENT = new SoundEvent(codHitmarker);
    private static final ResourceLocation csgoHitmarker = new ResourceLocation("loveClient", "csgo_hitmarker");
    ...
    private final Setting<String> customWatermark = this.register(new Setting<String>("WatermarkName", "loveClient b2"));
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "love2004", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public void onUpdate() {
        for (final Module module : loveClient.moduleManager.sortedModules) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case loveClient: {
                this.renderer.drawString("loveClient" + (this.modeVer.getValue() ? " b2" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case ILoveYou: {
                this.renderer.drawString("I.Love.You" + (this.modeVer.getValue() ? " b2" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case CUSTOM: {
                this.renderer.drawString(this.customWatermark.getValue() + (this.modeVer.getValue() ? " b2" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
        }
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? loveClient.moduleManager.alphabeticallySortedModules.size() : loveClient.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? loveClient.moduleManager.alphabeticallySortedModules.get(k) : loveClient.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "\u00a77" + ((module.getDisplayInfo() != null) ? (" [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]") : "");
                    final Color moduleColor = loveClient.moduleManager.moduleColorMap.get(module);
                ...
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? loveClient.moduleManager.alphabeticallySortedModules.size() : loveClient.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? loveClient.moduleManager.alphabeticallySortedModules.get(loveClient.moduleManager.alphabeticallySortedModules.size() - 1 - k) : loveClient.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "\u00a77" + ((module.getDisplayInfo() != null) ? (" [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]") : "");
                    final Color moduleColor = loveClient.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + "\u00a7f" + loveClient.serverManager.getServerBrand();
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : loveClient.potionManager.getOwnPotions()) {
                    final String text3 = this.altPotionsColors.getValue() ? loveClient.potionManager.getPotionString(effect) : loveClient.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + "\u00a7f" + loveClient.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + "\u00a7f" + loveClient.serverManager.getTPS();
            ...
            final String text = grayString + "Ping " + "\u00a7f" + (ServerModule.getInstance().isConnected() ? ServerModule.getInstance().getServerPing() : loveClient.serverManager.getPing()) + (this.MS.getValue() ? "ms" : "");
            ...
            final String fpsText = grayString + "FPS " + "\u00a7f" + Minecraft.debugFPS;
            final String text = grayString + "Ping " + "\u00a7f" + loveClient.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
        ...
        if (this.renderingUp.getValue()) {
            loveClient.notificationManager.handleNotifications(height - (k + 16));
        } else {
            loveClient.notificationManager.handleNotifications(height - (j + 16));
        }
        ...
        final String text4 = (this.direction.getValue() ? (loveClient.rotationManager.getDirection4D(false) + " ") : "") + (this.coords.getValue() ? coordinates : "") + "";
    ...
    public void renderLag() {
        final int width = this.renderer.scaledWidth;
        if (loveClient.serverManager.isServerNotResponding()) {
            final String text = ((this.lag.getValue() == LagNotify.GRAY) ? "\u00a77" : "\u00a7c") + "Server not responding: " + MathUtil.round(loveClient.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
