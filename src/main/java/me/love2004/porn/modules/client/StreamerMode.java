...
public class StreamerMode
    ...
    public void onUpdate() {
        if (this.window != null) {
            ...
            drawInfo.add("TPS: " + loveClient.serverManager.getTPS());
            drawInfo.add("Ping: " + loveClient.serverManager.getPing() + "ms");
            drawInfo.add("Speed: " + loveClient.speedManager.getSpeedKpH() + "km/h");
            ...
            String text = loveClient.rotationManager.getDirection4D(false);
            drawInfo.add("");
            drawInfo.add(text);
            drawInfo.add(coordinates);
            drawInfo.add("");
            for (Module module : loveClient.moduleManager.sortedModules) {
                String moduleName = TextUtil.stripColor(module.getFullArrayString());
                drawInfo.add(moduleName);
            }
            drawInfo.add("");
            for (PotionEffect effect : loveClient.potionManager.getOwnPotions()) {
                String potionText = TextUtil.stripColor(loveClient.potionManager.getColoredPotionString(effect));
