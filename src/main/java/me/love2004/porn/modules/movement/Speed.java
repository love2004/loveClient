...
public class Speed
    ...
    private boolean shouldReturn() {
        return loveClient.moduleManager.isModuleEnabled("Freecam") || loveClient.moduleManager.isModuleEnabled("Phase") || loveClient.moduleManager.isModuleEnabled("ElytraFlight") || loveClient.moduleManager.isModuleEnabled("Strafe") || loveClient.moduleManager.isModuleEnabled("Flight");
    }
        ...
        if (EntityUtil.isEntityMoving(Speed.mc.player) && !Speed.mc.player.collidedHorizontally && !BlockUtil.isBlockAboveEntitySolid(Speed.mc.player) && BlockUtil.isBlockBelowEntitySolid(Speed.mc.player)) {
            ...
            if (Speed.mc.player.posY >= this.startY + this.bounceHeight) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    loveClient.timerManager.setTimer(1.0f);
                }
            }
            if (Speed.mc.player.posY == this.startY) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    if (rnd) {
                        loveClient.timerManager.setTimer(1.3f);
                    } else {
                        loveClient.timerManager.setTimer(1.0f);
                    }
                }
