...
import me.love2004.porn.modules.player.BlockTweaks;
...
public class Surround
    ...
    public void onEnable() {
        ...
        if (this.center.getValue().booleanValue() && !loveClient.moduleManager.isModuleEnabled("Freecam")) {
            if (Surround.mc.world.getBlockState(new BlockPos(Surround.mc.player.getPositionVector())).getBlock() == Blocks.WEB) {
                loveClient.positionManager.setPositionPacket(Surround.mc.player.posX, this.startPos.getY(), Surround.mc.player.posZ, true, true, true);
            } else {
                loveClient.positionManager.setPositionPacket((double) this.startPos.getX() + 0.5, this.startPos.getY(), (double) this.startPos.getZ() + 0.5, true, true, true);
            }
        }
    ...
    private boolean placeBlocks(Vec3d pos, Vec3d[] vec3ds, boolean hasHelpingBlocks, boolean isHelping, boolean isExtending) {
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, this.raytrace.getValue())) {
                ...
                case 1: {
                    if ((this.switchMode.getValue() == InventoryUtil.Switch.SILENT || BlockTweaks.getINSTANCE().isEnable() && BlockTweaks.getINSTANCE().noBlock.getValue().booleanValue()) && (this.retries.get(position) == null || this.retries.get(position) < this.retryer.getValue())) {
                        this.placeBlock(position);
                        this.retries.put(position, this.retries.get(position) == null ? 1 : this.retries.get(position) + 1);
                        this.retryTimer.reset();
                        continue block6;
                    }
                    if (!this.extendMove.getValue().booleanValue() && loveClient.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= this.extender.getValue())
                        continue block6;
    ...
    private boolean check() {
        ...
        switch (this.movementMode.getValue()) {
            ...
            case LIMIT: {
                if (!(loveClient.speedManager.getSpeedKpH() > this.speed.getValue())) break;
                return true;
            }
            case OFF: {
                if (!(loveClient.speedManager.getSpeedKpH() > this.speed.getValue())) break;
        ...
        return loveClient.moduleManager.isModuleEnabled("Freecam") || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Surround.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockObsidian.class);
    }
