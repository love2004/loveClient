...
public class HoleFiller
    ...
    @Override
    public void onTick() {
        if (this.isEnable() && (this.blocksPerTick.getValue() != 1 || !this.rotate.getValue().booleanValue())) {
            this.doHoleFill();
        }
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (this.isEnable() && event.getStage() == 0 && this.blocksPerTick.getValue() == 1 && this.rotate.getValue().booleanValue()) {
            this.doHoleFill();
        }
    }
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            loveClient.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            loveClient.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = loveClient.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(loveClient.holeManager.getMidSafety());
            }
        }
        object = loveClient.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(loveClient.holeManager.getHoles());
        }
        for (BlockPos position : targets) {
            ...
            if ((placeability = BlockUtil.isPositionPlaceable(position, this.raytrace.getValue())) == 1 && (this.currentMode == Mode.WEBS || this.switchMode.getValue() == InventoryUtil.Switch.SILENT || BlockTweaks.getINSTANCE().isEnable() && BlockTweaks.getINSTANCE().noBlock.getValue().booleanValue()) && (this.currentMode == Mode.WEBS || this.retries.get(position) == null || this.retries.get(position) < 4)) {
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && loveClient.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
