        ...
        for (BlockPos position : this.getPositions()) {
            ...
            if (placeability == 1) {
                switch (this.currentMode) {
                    ...
                    case OBSIDIAN: {
                        if (this.switchMode.getValue() != InventoryUtil.Switch.SILENT && (!BlockTweaks.getINSTANCE().isEnable() || !BlockTweaks.getINSTANCE().noBlock.getValue().booleanValue()) || this.retries.get(position) != null && this.retries.get(position) >= 4)
                            break;
                        this.placeBlock(position);
