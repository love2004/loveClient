...
import me.love2004.porn.modules.combat.Killaura;
import me.love2004.porn.modules.player.Blink;
import me.love2004.porn.modules.player.FakePlayer;
import me.love2004.porn.modules.player.Freecam;
import me.love2004.porn.mixin.mixins.accessors.IEntityLivingBase;
...
public class EntityUtil implements Util {
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(EntityUtil.mc.player) || (entity instanceof EntityPlayer && loveClient.friendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && loveClient.friendManager.isFriend((EntityPlayer) entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, alpha / 255.0f);
            }
            final Killaura killaura = loveClient.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static boolean isFakePlayer(final EntityPlayer player) {
        ...
        if (freecam.isEnable() && playerID == 69420) {
            return true;
        }
        if (fakePlayer.isEnable()) {
        ...
        return blink.isEnable() && playerID == 6942069;
    }
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (loveClient.friendManager.isFriend(player) ? "\u00a7b" : "\u00a7r") + player.getName() + " " + distanceSB.toString() + " " + "\u00a7f" + loveClient.totemPopManager.getTotemPopString(player) + loveClient.potionManager.getTextRadarPotion(player), (int) EntityUtil.mc.player.getDistance(player));
