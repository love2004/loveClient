...
public class BlockUtil
    ...
    public static boolean placeBlockSmartRotate(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean isSneaking) {
        ...
        if (rotate) {
            loveClient.rotationManager.lookAtVec3d(hitVec);
        }
        BlockUtil.rightClickBlock(neighbour, hitVec, hand, opposite, packet);
        BlockUtil.mc.player.swingArm(EnumHand.MAIN_HAND);
        BlockUtil.mc.rightClickDelayTimer = 4;
    ...
    public static boolean placeBlock(final BlockPos pos, final int slot, final boolean rotate, final boolean rotateBack) {
        if (isBlockNotEmpty(pos) || mc.player.inventory.getStackInSlot(slot).getItem() == Item.getItemFromBlock(Blocks.WEB)) {
            for (final Entity entity : mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(pos))) {
            ...
            for (final EnumFacing enumFacing : values) {
                final Block neighborBlock = mc.world.getBlockState(pos.offset(enumFacing)).getBlock();
    ...
    public static boolean canPlaceBlock(final BlockPos pos) {
        if (isBlockNotEmpty(pos)) {
            ...
            final EnumFacing[] enumFacings = values = EnumFacing.values();
            for (final EnumFacing enumFacing : values) {
                if (!emptyBlocks.contains(mc.world.getBlockState(pos.offset(enumFacing)).getBlock()) && mc.player.getPositionEyes(mc.getRenderPartialTicks()).distanceTo(new Vec3d(pos.getX() + 0.5 + enumFacing.getXOffset() * 0.5, pos.getY() + 0.5 + enumFacing.getYOffset() * 0.5, pos.getZ() + 0.5 + enumFacing.getZOffset() * 0.5)) <= 4.25) {
                    return true;
                }
            }
        }
    ...
    public static void rotateClient(final double x, final double y, final double z) {
        ...
        player.rotationYaw += MathHelper.wrapDegrees(yaw - mc.player.rotationYaw);
        final EntityPlayerSP player2 = mc.player;
        player2.rotationPitch += MathHelper.wrapDegrees(pitch - mc.player.rotationPitch);
    }
